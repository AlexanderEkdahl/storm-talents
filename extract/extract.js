...
const validator = jsen({
        ...
        talents: {
            ...
            items: {
                ...
                items: {
                    ...
                    required: ["id", "icon", "name", "description"],
                    type: "object",
                },
                minItems: 2,
                maxItems: 5,
    ...
    required: ["id", "search", "talents", "name", "title"],
    type: "object",
});
...
const heroes = {};
libxml
    .parseXmlString(readFile("mods/heroesdata.stormmod/base.stormdata/GameData/ConfigData.xml"))
    .find("//HeroArray")
...
const documents = files.filter(file => file.endsWith(".xml")).map(readFile).map(libxml.parseXmlString);

const strings = {};

files.filter(file => file.endsWith("GameStrings.txt")).map(readFile).forEach(data => {
...
documents.forEach(document => {
    document.find("CHero").forEach(node => {
        ...
        talentNodes = node.find("TalentTreeArray").forEach(talentNode => {
...
const output = Object.keys(heroes)
    .map(id => {
        return {
            id: id,
            name: heroes[id].name,
...
fs.writeFileSync("../src/heroes.ts", `/* tslint:disable */\nexport default ${JSON.stringify(outputList, undefined, "  ")}`, {
    encoding: "utf8",
});
...
storm.extractFiles(
    "/Applications/Heroes of the Storm/",
    tmpDir,
...
const getValue = (node, subnodeName) => {
    if (!node) {
        return;
    }
...
const attributeValue = (node, attrName) => {
    if (!node) {
        return;
    }
